...
test "$URUQUIM_POLICY_ROWS" -eq 5 ||
  fail "the vendor policy lists $URUQUIM_POLICY_ROWS patch dispositions, not the 5 patches that ship. A patch with no recorded disposition is one nobody knows whether to re-apply."
