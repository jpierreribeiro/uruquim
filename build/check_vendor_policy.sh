...
# Twelve patches ship. The policy's disposition table and the vendor record must
...
test "$URUQUIM_POLICY_ROWS" -eq 12 ||
  fail "the vendor policy lists $URUQUIM_POLICY_ROWS patch dispositions, not the 12 patches that ship. A patch with no recorded disposition is one nobody knows whether to re-apply."
