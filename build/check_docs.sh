...
test "$URUQUIM_APP_COUNT" -eq 44 ||
  fail "the canonical application ledger is $URUQUIM_APP_COUNT, not 44; docs parity cannot be trusted"
...
grep -qE '\b44\b' <<<"$URUQUIM_AI_ACTIVE" ||
  fail "docs/ai-context.md does not state the 44-symbol application ledger"
grep -qE '\b46\b' <<<"$URUQUIM_AI_ACTIVE" ||
  fail "docs/ai-context.md does not state the 46-symbol union"
