...
test "$URUQUIM_APP_COUNT" -eq 37 ||
  fail "the canonical application ledger is $URUQUIM_APP_COUNT, not 37; docs parity cannot be trusted"
...
grep -qE '\b37\b' <<<"$URUQUIM_AI_ACTIVE" ||
  fail "docs/ai-context.md does not state the 37-symbol application ledger"
grep -qE '\b39\b' <<<"$URUQUIM_AI_ACTIVE" ||
  fail "docs/ai-context.md does not state the 39-symbol union"
...
for URUQUIM_DOC in "${URUQUIM_ACTIVE_DOCS[@]}"; do
  ...
  for URUQUIM_FUTURE in 'web\.group\b' \
    'web\.header\(' 'web\.bearer_token\b' 'web\.state\b' \
