...
# ---------------------------------------------------------------------------
# 4. The ledgers still add up: 51 application + 2 test-support = 53
...
[ "$URUQUIM_FREEZE_APP_COUNT" -eq 51 ] ||
  fail "the application ledger holds $URUQUIM_FREEZE_APP_COUNT symbols, not the recorded 51 (the Phase-1 32, the Phase-2 twelve, the Phase-3 six, plus WP44 stop, spec §9.2)"
[ "$URUQUIM_FREEZE_TS_COUNT" -eq 2 ] ||
  fail "the test-support ledger holds $URUQUIM_FREEZE_TS_COUNT symbols, not the frozen 2"
[ "$URUQUIM_FREEZE_TOTAL" -eq 53 ] ||
  fail "the exported union is $URUQUIM_FREEZE_TOTAL, not the recorded 53"
...
[ "$URUQUIM_FREEZE_REF_COUNT" -ge 53 ] ||
  fail "$URUQUIM_FREEZE_MANIFEST carries only $URUQUIM_FREEZE_REF_COUNT resolvable evidence citations for 53 recorded symbols; the matrix is incomplete"
...
echo "freeze: evidence matrix -> $URUQUIM_FREEZE_REF_COUNT citations resolved, all 53 symbols present, none NOT_FROZEN"
