...
# ---------------------------------------------------------------------------
# 4. The ledgers still add up: 57 application + 2 test-support = 59
...
[ "$URUQUIM_FREEZE_APP_COUNT" -eq 57 ] ||
  fail "the application ledger holds $URUQUIM_FREEZE_APP_COUNT symbols, not the recorded 57 (the Phase-1 32, the Phase-2 twelve, the Phase-3 six, plus WP44 stop, WP48 client_ip/trust_proxies, WP49 secure_headers, WP50 refused_connections and WP60 cors/Cors_Options)"
[ "$URUQUIM_FREEZE_TS_COUNT" -eq 2 ] ||
  fail "the test-support ledger holds $URUQUIM_FREEZE_TS_COUNT symbols, not the frozen 2"
[ "$URUQUIM_FREEZE_TOTAL" -eq 59 ] ||
  fail "the exported union is $URUQUIM_FREEZE_TOTAL, not the recorded 59"
...
[ "$URUQUIM_FREEZE_REF_COUNT" -ge 59 ] ||
  fail "$URUQUIM_FREEZE_MANIFEST carries only $URUQUIM_FREEZE_REF_COUNT resolvable evidence citations for 59 recorded symbols; the matrix is incomplete"
...
echo "freeze: evidence matrix -> $URUQUIM_FREEZE_REF_COUNT citations resolved, all 59 symbols present, none NOT_FROZEN"
