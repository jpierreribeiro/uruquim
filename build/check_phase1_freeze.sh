...
# ---------------------------------------------------------------------------
# 4. The ledgers still add up: 44 application + 2 test-support = 46
...
[ "$URUQUIM_FREEZE_APP_COUNT" -eq 44 ] ||
  fail "the application ledger holds $URUQUIM_FREEZE_APP_COUNT symbols, not the recorded 44 (32 frozen Phase-1 + WP17 + WP18 + WP19 + WP20 observer + WP22 logger + WP23 request_id, spec §9.2)"
[ "$URUQUIM_FREEZE_TS_COUNT" -eq 2 ] ||
  fail "the test-support ledger holds $URUQUIM_FREEZE_TS_COUNT symbols, not the frozen 2"
[ "$URUQUIM_FREEZE_TOTAL" -eq 46 ] ||
  fail "the exported union is $URUQUIM_FREEZE_TOTAL, not the recorded 46"
...
[ "$URUQUIM_FREEZE_REF_COUNT" -ge 46 ] ||
  fail "$URUQUIM_FREEZE_MANIFEST carries only $URUQUIM_FREEZE_REF_COUNT resolvable evidence citations for 46 recorded symbols; the matrix is incomplete"
...
echo "freeze: evidence matrix -> $URUQUIM_FREEZE_REF_COUNT citations resolved, all 46 symbols present, none NOT_FROZEN"
