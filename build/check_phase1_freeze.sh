...
# ---------------------------------------------------------------------------
# 4. The ledgers still add up: 45 application + 2 test-support = 47
...
#    amendment, decided under the ADR-029 delegation. All ratified.)
# ---------------------------------------------------------------------------
...
[ "$URUQUIM_FREEZE_APP_COUNT" -eq 45 ] ||
  fail "the application ledger holds $URUQUIM_FREEZE_APP_COUNT symbols, not the recorded 45 (32 frozen Phase-1 + WP17 + WP18 + WP19 + WP20 observer + WP22 logger + WP23 request_id + WP34 route, spec §9.2)"
[ "$URUQUIM_FREEZE_TS_COUNT" -eq 2 ] ||
  fail "the test-support ledger holds $URUQUIM_FREEZE_TS_COUNT symbols, not the frozen 2"
[ "$URUQUIM_FREEZE_TOTAL" -eq 47 ] ||
  fail "the exported union is $URUQUIM_FREEZE_TOTAL, not the recorded 47"
...
[ "$URUQUIM_FREEZE_REF_COUNT" -ge 47 ] ||
  fail "$URUQUIM_FREEZE_MANIFEST carries only $URUQUIM_FREEZE_REF_COUNT resolvable evidence citations for 47 recorded symbols; the matrix is incomplete"
...
echo "freeze: evidence matrix -> $URUQUIM_FREEZE_REF_COUNT citations resolved, all 47 symbols present, none NOT_FROZEN"
