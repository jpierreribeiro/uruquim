...
# ---------------------------------------------------------------------------
# 4. The ledgers still add up: 53 application + 2 test-support = 55
...
[ "$URUQUIM_FREEZE_APP_COUNT" -eq 53 ] ||
  fail "the application ledger holds $URUQUIM_FREEZE_APP_COUNT symbols, not the recorded 53 (the Phase-1 32, the Phase-2 twelve, the Phase-3 six, plus WP44 stop and WP48 client_ip/trust_proxies, spec §9.2)"
[ "$URUQUIM_FREEZE_TS_COUNT" -eq 2 ] ||
  fail "the test-support ledger holds $URUQUIM_FREEZE_TS_COUNT symbols, not the frozen 2"
[ "$URUQUIM_FREEZE_TOTAL" -eq 55 ] ||
  fail "the exported union is $URUQUIM_FREEZE_TOTAL, not the recorded 55"
...
[ "$URUQUIM_FREEZE_REF_COUNT" -ge 55 ] ||
  fail "$URUQUIM_FREEZE_MANIFEST carries only $URUQUIM_FREEZE_REF_COUNT resolvable evidence citations for 55 recorded symbols; the matrix is incomplete"
...
echo "freeze: evidence matrix -> $URUQUIM_FREEZE_REF_COUNT citations resolved, all 55 symbols present, none NOT_FROZEN"
