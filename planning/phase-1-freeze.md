...
| G-06 | Backend stays private | **PASS** | `build/check_public_api.sh::odin_http_adapter.odin` pins the backend import to one file and bans `odin-http`/`nbio`/`laytan` in `web/` and `examples/`; `tests/wp9-public-surface/contract_test.odin::wp9_added_no_transport_surface` |
