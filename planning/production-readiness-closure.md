...
| **C-05** | Write & saturation observability | IMPLEMENTATION | ◑ **PERIMETER 6 DONE, PERIMETER 7 SPECIFIED.** `planning/closure-saturation-and-write-observability.md` + `tests/c05-saturation`, gated by `build/check_c05_controls.sh`. **The lab found a third production-blocking defect: F-C05-1** — `handler_lane_enter`'s accept-cancel spin is unbounded, wedging shutdown in **4 runs of 6** on pristine `main` (`web.stop` did not return in 60 s against a 3 s `max_drain_time`), because a lane parked in the spin never reaches `_server_thread_shutdown` and `serve` waits for every lane. Fixed by vendored patch 27; 11/11 green, and the formerly-wedging runs now return at exactly `max_drain_time`. **This reclassifies C-01's F-C01-3**, which had called the same spin acceptable on reasoning rather than on a test. **Perimeter 6 measured: the Handler lane binds FIRST**, at 4 concurrent clients — not the 20-slot connection budget, not the backlog. **Perimeter 7 (write observability) is specified (`web.Server_Stats`/`web.stats()`, +2 ledger) and deferred** to its own WP, for the same reason as C-04's limit: a twelve-file ledger amendment does not belong at the tail of an audit phase. |
